...
Additionally, since toasts are only shown in the context of an application's window, they are only appropriate for feedback that is useful while the application is being used. If it is important that the message to be visible while the app is not being used, a :doc:`notification <notifications>` is probably a better choice.
