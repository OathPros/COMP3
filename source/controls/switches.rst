...
When the control does not turn a function on or off, or when a function does not clearly have an on/off nature, a :doc:`check box <checkboxes>` is a more appropriate option. If in doubt, use switches for important configuration options, and check boxes for minor sub-options.
